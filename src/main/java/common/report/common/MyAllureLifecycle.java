...
public class MyAllureLifecycle {

    private static  Logger LOGGER = LoggerFactory.getLogger(MyAllureLifecycle.class);

    private  AllureResultsWriter writer;

    private  AllureStorage storage;

    private  AllureThreadContext threadContext;

    private  LifecycleNotifier notifier;

    /**
     * Creates a new lifecycle with default results writer. Shortcut
     * for {@link #MyAllureLifecycle(AllureResultsWriter)}
     */
    public MyAllureLifecycle() {
     ...
     */
    public MyAllureLifecycle( AllureResultsWriter writer) {
     ...
     * @param writer the results writer.
     */
    MyAllureLifecycle( AllureResultsWriter writer,  LifecycleNotifier lifecycleNotifier) {
    ...
    public void updateTestContainer( String uuid,  Consumer<TestResultContainer> update) {
         Optional<TestResultContainer> found = storage.getContainer(uuid);
        if (!found.isPresent()) {
            LOGGER.error("Could not update test container: container with uuid {} not found", uuid);
            return;
        }
         TestResultContainer container = found.get();
    ...
    public void updateFixture( Consumer<FixtureResult> update) {
         Optional<String> root = threadContext.getRoot();
        if (!root.isPresent()) {
    ...
    public void updateFixture( String uuid,  Consumer<FixtureResult> update) {
         Optional<FixtureResult> found = storage.getFixture(uuid);
        if (!found.isPresent()) {
    ...
    public boolean setCurrentTestCase( String uuid) {
         Optional<TestResult> found = storage.getTestResult(uuid);
        if (!found.isPresent()) {
            return false;
        }
        threadContext.clear();
    ...
    public void startStep( String uuid,  StepResult result) {
         Optional<String> current = threadContext.getCurrent();
    ...
    public void updateStep( String uuid,  Consumer<StepResult> update) {
         Optional<StepResult> found = storage.getStep(uuid);
        if (!found.isPresent()) {
    ...
    public void stopStep() {
         String root = threadContext.getRoot().orElse(null);
         Optional<String> current = threadContext.getCurrent()
    ...
    public void addAttachment( String name,  String type,
                               String fileExtension,  byte[] body) {
     ...
     */
    public void addAttachment( String name,  String type,
                               String fileExtension,  InputStream stream) {
    ...
    private static SystemResultsWriter getDefaultWriter() {
        ...
        return new SystemResultsWriter(Paths.get(path));
    }
